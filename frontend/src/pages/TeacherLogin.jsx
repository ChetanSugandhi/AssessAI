...
const AuthForm = () => {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#001a2c] to-slate-950">
      <div className={`bg-slate-800 rounded-[30px] shadow-lg relative overflow-hidden w-[768px] max-w-full min-h-[480px]  ''`}>
        {/* Sign In Form */}
        <div className={`absolute top-0 h-full transition-all duration-600 ease-in-out left-0 w-1/2 z-[2]'
          }`}>
          <form className="bg-slate-800 h-full flex flex-col items-center justify-center px-10">
        ...
        <div className={`absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-all duration-600 ease-in-out rounded-l-[150px] rounded-bl-[100px] z-[1000] `}>
          <div className={`bg-gradient-to-r from-cyan-500 to-cyan-600 text-white relative -left-full h-full w-[200%] transform translate-x-0'
            } transition-transform duration-600 ease-in-out`}>
            {/* Toggle Left Panel */}
            <div className={`absolute w-1/2 h-full flex flex-col items-center justify-center px-8 text-center transform -translate-x-[200%]'
              } transition-transform duration-600 ease-in-out`}>
            ...
            {/* Toggle Right Panel */}
            <div className={`absolute right-0 w-1/2 h-full flex flex-col items-center justify-center px-8 text-center transform translate-x-0'
              } transition-transform duration-600 ease-in-out`}>
              <img src='teacher.png' alt="Learning" className="w-50 h-50" />
              <p className="my-4 text-cyan-100">Welcome, Teachers!</p>
              <p className="text-cyan-100">Your tools for enlightenment await</p>
            </div>
          </div>
        </div>
      </div>
